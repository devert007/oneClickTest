	...
	downloadDocument: async (fileId, filename) => {
		try {
			...
			if (error.response?.status === 404) {
				...
				errorMessage =
					"Текст документа недоступен. Возможно, документ был загружен до реализации системы индексации.";
			}
...
export const testGenerationAPI = {
	...
	saveTest: async (
		...
		try {
			...
			return await testGenerationAPI.saveTestOld(
				testContent,
				filename,
				documentId,
				sessionId,
	...
	saveTestOld: async (
		...
		try {
			...
			const result = await testPDFAPI.uploadTestPDF(
				file,
				documentId,
				sessionId,
			);
