    ...
    const handleUploadNewDocument = async () => {
        ...
        try {
            ...
            if (!uniquenessCheck.is_unique) {
                setUploadDocumentStatus(`Документ не уникален: ${uniquenessCheck.message || uniquenessCheck.detail}`);
                return;
            }

            // Загружаем документ
            const result = await documentAPI.uploadDocument(newDocumentFile);
            setUploadDocumentStatus(`Документ "${newDocumentFile.name}" успешно загружен! ID: ${result.file_id}`);
            ...
            setUploadDocumentStatus(`Ошибка при загрузке документа: ${error.response?.data?.detail || error.message}`);
        } finally {
                        ...
                        <div className="params-grid">
                            <div className="form-group">
                                ...
                                <div className="upload-document-section">
                                    ...
                                    >
                                        {showUploadForm ? '✖ Отменить загрузку' : '📤 Загрузить новый документ'}
                                    </button>

                                    {/* Форма загрузки нового документа */}
                                    {showUploadForm && (
                                        <div className="new-document-upload" style={{
                                            marginTop: '15px',
                                            padding: '15px',
                                            ...
                                            <div className="upload-area">
                                                ...
                                                {newDocumentFile && (
                                                    <div className="selected-file-info" style={{
                                                        ...
                                                        <br />
                                                        <small>Размер: {Math.round(newDocumentFile.size / 1024)} KB</small>
                                                    </div>
                                                )}
                                                ...
                                                >
                                                    {isUploadingDocument ? '🔄 Загрузка...' : '📤 Загрузить'}
                                                </button>
                                            ...
                                            {uploadDocumentStatus && (
                                                <div className={`upload-status ${uploadDocumentStatus.includes('Ошибка') ? 'error' : 'success'}`} style={{
                                                    marginTop: '10px',
                                                    padding: '10px',
                                                    borderRadius: '4px',
                                                    fontSize: '0.9em'
                                            ...
                                            <div className="supported-formats-info" style={{
                                                marginTop: '10px',
                                                fontSize: '0.8em',
                                                color: '#666'
                                            }}>
                                ...
                                <div className="document-help" style={{ fontSize: '0.8em', color: '#666', marginTop: '5px' }}>
                                    {selectedDocument ?
